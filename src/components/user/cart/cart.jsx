    ...
    <div>
      ...
      <div className="mt-10 grid lg:grid-cols-2 gap-x-8 gap-y-8 items-center px-40 py-10">
        {cart !=="Missing Username" && cart.map((product) => {
          ...
          return (
            <a key={`cart${detail.id}`} href="#" className="flex flex-col items-center bg-white rounded-lg border shadow-md md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
              <img className="object-cover w-full h-96 rounded-t-lg md:h-auto md:w-48 md:rounded-none md:rounded-l-lg" src={detail.thumbnail} alt="" />
              <div className="flex flex-col justify-between p-4 leading-normal">
                  <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{detail.name}</h5>
                  <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">Marca: {detail.brand}</p>
                  <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">Modelo: {detail.model}</p>
              </div>
              <CardActions disableSpacing >
              <IconButton aria-label="Add to cart" onClick={() => {
                if (isFavorite(detail.id)) return unSetFavorite(localStorage.userName, detail.id)
                setFavorite(localStorage.userName, detail.id)
              }
              }>
                {favorites != "Missing Username" ? (isFavorite(detail.id) 
                ...
                <NumberInput size='sm'  defaultValue={product.amount} min={0} onChange={
                  (value) =>{ 
                   if (userState) {
