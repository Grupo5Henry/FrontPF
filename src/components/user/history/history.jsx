...
export default function History() {
  ...
  console.log(history);
  return (
    <div class="flex flex-wrap flex-row justify-center mt-20 gap-6 w-11/12">
      {history.map((e) => (
        <div class="relative inline-block duration-300 ease-in-out transition-transform transform hover:-translate-y-2 w-2/12">
          <div class="shadow p-4 rounded-lg bg-white">
            <Link to={`/home/detail/${e.id}`}>
              ...
              <div class="grid grid-cols-2 grid-rows-2 gap-4 mt-8">
                <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                  ...
                  <span class="mt-2 xl:mt-0">${e.price}</span>
                </p>
              </div>
            </Link>
            ...
            <div class="grid grid-cols-2 mt-3">
              <div class="flex items-center">
                <button onClick={()=> dispatch(deleteHistory(e.id))} class="ml-2 text-gray-800 line-clamp-1">Eliminar</button>
              </div>
            </div>
          </div>
        </div>
